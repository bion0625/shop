...
<html>
	<head>
		...
		<script type="text/javascript">
			function modify(m) {
				...
				if(pwd == c_pwd){
					location.href='loss_modify_form.do?idx='+m;
					return;
				}
					alert("비밀번호가 틀립니다");
			}
			function del(d) {
				...
				location.href='loss_del.do?idx='+d;
			}
		</script>
	</head>
	<body>
	<div id="main_box">
		...
		<c:if test="${ vo.step eq 0 }">
			...
			<tr>
				<td align="left">
					<input value="목록" type="button" onclick="location.href='loss_list.do?page=${param.page}'">
				</td>
				<td align="right">
					<input value="수정" type="button" onclick="modify(${vo.idx});">
					<input value="삭제" type="button" onclick="del(${vo.idx});">
					<input value="댓글쓰기" type="button" onclick="location.href='loss_reply_form.do?idx=${vo.idx}'">
				</td>
			</tr>
		</c:if>
		<c:if test="${ vo.step > 0 }">
